...
export class StateHandlerRoom extends Room<State> {
    ...
    gameIsStared: boolean = false;

    awaitStart: Delayed;

    async onJoin (client: Client, data : any) {
        try {
            const response = await axios.post(Library.getDeckURL, {key: Library.phpKEY, userID: data.id});
    ...
    onLeave (client) {
        ...
        if(this.gameIsStared === false && this.awaitStart !== undefined && this.awaitStart.active){
            this.broadcast("CancelStart");
