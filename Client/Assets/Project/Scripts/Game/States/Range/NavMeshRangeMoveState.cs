...
{
    [CreateAssetMenu(fileName = "_NavMeshRangeMoveState", menuName = "UnitState/" + nameof(NavMeshRangeMoveState))]
    public class NavMeshRangeMoveState : UnitStateNavMeshMove
    {
        ...
        {
            bool hasEnemy = Map.Current.TryGetNearestAnyUnit(_unit.transform.position, _unit.IsPlayer, out Unit enemy,
                out float distance);
            if (hasEnemy == false)
                return false;
